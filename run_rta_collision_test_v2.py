#!/usr/bin/env python3
"""
run_rta_collision_test_v2.py - 修正版

修正:
1. GRU 用安全包线边界判断，不是简单阈值
2. R3 三个模块完整实现 (激活+OOD+梯度)
3. 放宽阈值减少误报
"""

import sys
sys.argv = ['run_rta_collision_test_v2.py']
...
class Region1Monitor:
    ...
    def check(self, qpos, qvel):
        ...
        if qpos_margin < 0:
            ...
            risk_score = 1.0 - (qpos_margin / 0.3)
...
class Region2Monitor:
    ...
    def __init__(self, model_path, safe_boundaries=None):
        ...
        # 安全包线边界 (16 个方向的最大允许支撑函数值)
        if safe_boundaries is None:
            self.safe_boundaries = np.ones(16) * 1.5  # 默认 1.5
        else:
    ...
    def check(self, qpos, qvel, action):
        ...
        返回:
        - safe: bool 是否安全
        - risk_score: float 风险分数 (max(support/boundary))
        """
...
class Region3Monitor:
    ...
    def compute_gradient(self, policy, qpos_tensor, image, action_idx=0):
        """计算梯度贡献 φ = qpos × ∂a/∂qpos"""
        qpos_tensor.requires_grad_(True)
        action_out = policy(qpos_tensor, image)
        action_now = action_out[:, action_idx, :]
        ...
        contrib = qpos_np[None, :] * gradient
        return contrib
    
    def check(self, action, qpos, qvel, activations, gradient_contrib=None):
        """
        三模块检测:
        1. 激活链路 (汉明距离)
        2. OOD 检测 (马氏距离)
        ...
        # === 3. 梯度贡献 ===
        if gradient_contrib is not None:
            total_contrib = np.abs(gradient_contrib).sum()
            legal_contrib = 0.0
            for action_dim in range(14):
                for state_dim in F_legal[action_dim]:
                    legal_contrib += np.abs(gradient_contrib[action_dim, state_dim])
        ...
        alerts = {
            ...
            'logic': S_logic < self.logic_threshold,
        }
        
        # 风险分数
        risk_activation = D_ham / threshold_activation if threshold_activation > 0 else 0
        risk_ood = D_ood / self.ood_threshold if self.ood_threshold > 0 else 0
        risk_logic = 1.0 - S_logic
        
        triggered = alerts['activation'] or alerts['ood'] or alerts['logic']
        ...
        return not triggered, max(risk_activation, risk_ood, risk_logic), alerts, scores
...
class CollisionTester:
    def __init__(self, r1_config, r2_model_path, r2_boundaries, r3_model_dir, ckpt_dir, output_dir):
        ...
        print("="*70)
        print("三层 RTA 碰撞检测测试 (修正版)")
        print("="*70)
        
        self.region1 = Region1Monitor(**r1_config)
        print(f"  ✓ Region 1: 物理硬约束 (qpos:±{r1_config['qpos_max']}, qvel:{r1_config['qvel_max']})")
        
        self.region2 = Region2Monitor(r2_model_path, safe_boundaries=r2_boundaries)
        print(f"  ✓ Region 2: GRU 可达性 (安全边界={r2_boundaries[0]})")
        
        self.region3 = Region3Monitor(r3_model_dir)
        print(f"  ✓ Region 3: 感知异常 (激活+OOD+ 梯度)")
    ...
    def run_episode(self, episode_id, scene: ObstacleScene):
        ...
        trial_data = {'episode_id': episode_id, 'scene': scene.obstacle_type, 'steps': []}
        
        tp, fp, tn, fn = 0, 0, 0, 0
        first_alert, first_danger = None, None
        ...
        for t in tqdm(range(self.max_steps), desc=f"Ep {episode_id} [{scene.obstacle_type}]", leave=False):
            ...
            alerts = {'r1': not safe_r1, 'r2': not safe_r2, 'r3': not safe_r3}
            ...
            # 统计混淆矩阵
            if collision:
                if first_danger is None: first_danger = t
                if alerts['any']:
                    tp += 1
                    if first_alert is None: first_alert = t
                else:
                    fn += 1
            else:
                if alerts['any']:
            ...
            step_data = {
                't': t, 'qpos': qpos.tolist(), 'action': action.tolist(),
                'reward': ts.reward if ts.reward is not None else 0,
                'collision': collision, 'distance_to_obstacle': distance,
                'alert_any': alerts['any'], 'alert_r1': alerts['r1'],
                'alert_r2': alerts['r2'], 'alert_r3': alerts['r3'],
                'risk_r1': risk_r1, 'risk_r2': risk_r2, 'risk_r3': risk_r3,
            }
            if scores_r3:
                step_data.update(scores_r3)
        ...
        trial_data['confusion_matrix'] = {'tp': tp, 'fp': fp, 'tn': tn, 'fn': fn}
        trial_data['metrics'] = {
        ...
        trial_data['warning_lead_time'] = (first_danger - first_alert) if (first_alert is not None and first_danger is not None) else None
    ...
    def save_trial(self, trial_data, episode_id):
        ...
        cm = trial_data['confusion_matrix']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='./outputs/rta_collision_tests_v2')
