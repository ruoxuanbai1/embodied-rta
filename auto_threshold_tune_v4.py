...
for idx, fp in enumerate(sample_files):
    ...
    for i, step in enumerate(data):
        ...
        all_danger.append(step["ground_truth"]["actual_danger"])
        all_r1.append(step["region1"]["alarm"])
        all_s_logic.append(step["region3"]["scores"].get("S_logic", 1.0))
        all_d_ham.append(step["region3"]["scores"].get("D_ham", 0.0))
        all_d_ood.append(step["region3"]["scores"].get("D_ood", 0.0))
    ...
    log("  已处理 %d/%d 集 (%d 步)" % (idx+1, len(sample_files), file_steps))

log("  预计算完成: %d 步" % len(all_danger))
del model
gc.collect()
...
log("\n[4/5] 构建 numpy 数组...")
n = len(all_danger)
margins_arr = np.array(all_margin)
danger_arr = np.array(all_danger)
r1_arr = np.array(all_r1)
s_logic_arr = np.array(all_s_logic)
d_ham_arr = np.array(all_d_ham)
d_ood_arr = np.array(all_d_ood)
del all_danger, all_r1, all_s_logic, all_d_ham, all_d_ood, all_margin
gc.collect()
