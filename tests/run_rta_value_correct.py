...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'rta_value_correct'
...
N_BASE_SCENARIOS = 3  # empty, sparse, dense
N_RANDOM_SCENARIOS = 100  # 100 组随机障碍物
N_RUNS_BASE = 10
N_RUNS_RANDOM = 3
...
print(f'Base scenarios: {N_BASE_SCENARIOS}')
print(f'Random scenarios: {N_RANDOM_SCENARIOS}')
print(f'Faults: {len(FAULT_TYPES)}')
print(f'RTAs: {len(RTA_CONFIGS)}')

total = (N_BASE_SCENARIOS * N_RUNS_BASE + N_RANDOM_SCENARIOS * N_RUNS_RANDOM) * len(FAULT_TYPES) * len(RTA_CONFIGS)
print(f'Total: {total:,}')
print()
...
# 生成随机场景
random_scenes = []
for scene_id in range(N_RANDOM_SCENARIOS):
    scene_type = random.choice(['empty', 'sparse', 'dense'])
    random_scenes.append((scene_id, scene_type))

print(f'Generated {len(random_scenes)} random scenes')
        ...
        for fault in FAULT_TYPES:
            for rta_id, rta_name, r1, r2, r3 in RTA_CONFIGS:
                for r in range(N_RUNS_BASE):
                    seed = hash(f'base_{scene_type}_{fault["id"]}_{rta_id}_{r}') % (2**31)
                    futures.append(ex.submit(run_trial, -1, scene_type, fault, rta_id, rta_name, r1, r2, r3, seed))
    
    # 随机场景
    for scene_id, scene_type in random_scenes:
        for fault in FAULT_TYPES:
            for rta_id, rta_name, r1, r2, r3 in RTA_CONFIGS:
                for r in range(N_RUNS_RANDOM):
                    seed = hash(f'random_{scene_id}_{fault["id"]}_{rta_id}_{r}') % (2**31)
...
if len(all_results) > 0:
    ...
    print(f'Pure_VLA:       {pure_vla_rate*100:5.1f}% (baseline)')
    for rta in ['Ours_Full', 'R1_R3', 'R2_R3', 'CBF_QP', 'Recovery_RL']:
        if rta in fault_df['rta'].values:
            rate = fault_df[fault_df['rta']==rta]['success'].mean()
            improvement = (rate - pure_vla_rate) / (1 - pure_vla_rate) * 100 if pure_vla_rate < 1 else 0
            print(f'{rta:15s} {rate*100:5.1f}% (+' + f'{improvement:.1f}' + '% vs Pure_VLA)')
    ...
    # 干预率
    print('\nIntervention Rate:')
    interventions = fault_df.groupby('rta')['interventions'].mean().sort_values(ascending=False)
