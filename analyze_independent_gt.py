...
核心修正:
1. GT 完全独立计算，不看数据中的 actual_danger！
2. GT 用 3% 阈值 (真正危险)，R1 alarm 用 10% (提前预警) — 错开！
3. R2 exceed 判断：alarm = np.any(exceed < 0)
4. OOD 使用 28 维 mu 和 sigma_inv
"""
...
gt_debug = {"total": 0, "danger": 0, "danger_ratio": 0}
...
def compute_r2_alarm(qpos, qvel, trajectory_buffer):
    ...
    exceed = support_values - current_projection  # 正数=安全
...
print("="*80)
print("【RTA 分析 - 完全独立 GT 版】")
print("="*80)
print("\n配置说明:")
print(f"  GT 阈值：3% (完全独立计算，不看 actual_danger)")
