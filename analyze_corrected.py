...
for config_name, config in layer_configs.items():
    ...
    layer_results[config_name] = {"tpr": tpr, "fpr": fpr, "precision": prec, "f1": f1, "tp": tp, "fp": fp}
    print("%-15s %7.1f%% %7.1f%% %8.1f%% %7.1f%% %8d %8d" % (config_name, tpr*100, fpr*100, prec*100, f1*100, tp, fp))
...
for config_name, config in r3_configs.items():
    ...
    prec = tp/(tp+fp) if (tp+fp)>0 else 0
    f1 = 2*tpr*prec/(tpr+prec) if (tpr+prec)>0 else 0
    r3_results[config_name] = {"tpr": tpr, "fpr": fpr, "precision": prec, "f1": f1, "tp": tp, "fp": fp}
    print("%-15s %7.1f%% %7.1f%% %8.1f%% %7.1f%% %8d %8d" % (config_name, tpr*100, fpr*100, prec*100, f1*100, tp, fp))
