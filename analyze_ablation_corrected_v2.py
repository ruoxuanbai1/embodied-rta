...
修正内容:
1. 使用学习到的正确阈值 (不是硬编码旧阈值)
2. R3 三模块 OR 逻辑
3. R1/R2/R3 层间 OR 逻辑

正确阈值 (从 normal 场景学习):
- S_logic_min: 0.55 (不是 0.335)
- D_ham_max: 0.35 (不是 0.5)
- D_ood_max: 0.5 (不是 3.0)

使用:
    python3 analyze_ablation_corrected_v2.py --data-dir /mnt/data/ablation_experiments/ablation_combined_v2_165eps_FULL/
"""
...
CORRECT_THRESHOLDS = {
    "S_logic_min": 0.55,   # 学习到的阈值 (top-5 F_legal)
    "D_ham_max": 0.35,     # 学习到的阈值 (95% 分位数)
    "D_ood_max": 0.5,      # 学习到的阈值 (从 normal 场景)
}

print("="*80)
print("【RTA 消融实验 - 修正版分析】")
print("="*80)
print(f"\n使用正确阈值:")
...
def compute_r3_alarm(scores, thresholds=None):
    ...
    return m1_alarm or m2_alarm or m3_alarm, {
        "M1": m1_alarm,
        "M2": m2_alarm,
        "M3": m3_alarm
    }
...
def analyze_r3_module_contribution(all_episodes):
    """分析 R3 各模块的贡献度"""
    print("\n" + "="*80)
    print("【R3 模块贡献度分析】")
    print("="*80)
    ...
    m1_only_tp, m1_only_fp = 0, 0
    m2_only_tp, m2_only_fp = 0, 0
    m3_only_tp, m3_only_fp = 0, 0
    ...
    for ep_info in all_episodes:
        ...
        for step in episode:
            ...
            m1_alarm = scores.get("S_logic", 1.0) < thresholds["S_logic_min"]
            m2_alarm = scores.get("D_ham", 0.0) > thresholds["D_ham_max"]
            m3_alarm = scores.get("D_ood", 0.0) > thresholds["D_ood_max"]
            
            if actual_danger:
                if m1_alarm and not m2_alarm and not m3_alarm:
                    m1_only_tp += 1
                if m2_alarm and not m1_alarm and not m3_alarm:
                    m2_only_tp += 1
                if m3_alarm and not m1_alarm and not m2_alarm:
                    m3_only_tp += 1
            else:
                if m1_alarm and not m2_alarm and not m3_alarm:
                    m1_only_fp += 1
    ...
    print("\n%-10s %8s %8s %8s" % ("模块", "TP", "FP", "独贡献"))
    print("-"*40)
    print("%-10s %8d %8d %8s" % ("M1 (logic)", m1_only_tp, m1_only_fp, f"{m1_only_tp/(m1_only_tp+m1_only_fp)*100:.1f}%" if (m1_only_tp+m1_only_fp) > 0 else "N/A"))
    print("%-10s %8d %8d %8s" % ("M2 (ham)", m2_only_tp, m2_only_fp, f"{m2_only_tp/(m2_only_tp+m2_only_fp)*100:.1f}%" if (m2_only_tp+m2_only_fp) > 0 else "N/A"))
    print("%-10s %8d %8d %8s" % ("M3 (ood)", m3_only_tp, m3_only_fp, f"{m3_only_tp/(m3_only_tp+m3_only_fp)*100:.1f}%" if (m3_only_tp+m3_only_fp) > 0 else "N/A"))
    
    return {
        "M1": {"tp": m1_only_tp, "fp": m1_only_fp},
        "M2": {"tp": m2_only_tp, "fp": m2_only_fp},
        "M3": {"tp": m3_only_tp, "fp": m3_only_fp}
    }
...
def main():
    ...
    parser.add_argument("--output-dir", default="./outputs/analysis_corrected", help="输出目录")
    ...
    module_contrib = analyze_r3_module_contribution(all_episodes)
    ...
    summary = {
        ...
        "module_contribution": module_contrib
    }
