...
def save_results():
    ...
    with open(os.path.join(output_dir, "r3_current.json"), "w") as f:
        json.dump({"thresholds": CORRECT_THRESHOLDS, "episodes": ep_count, "steps": total_steps, "r3_ablation": r3_results}, f, indent=2)
            ...
            try:
                ...
                with open(filepath) as f:
                    ...
                    for line in f:
                        ...
                        if "ground_truth" not in step or "region3" not in step: continue
                        
                        danger = step["ground_truth"].get("actual_danger", False)
                        scores = step["region3"].get("scores", {})
                        
                        for config_name, config in r3_configs.items():
                            alarms = []
                            if config.get("M1"):
                                alarms.append(scores.get("S_logic", 1.0) < CORRECT_THRESHOLDS["S_logic_min"])
                            if config.get("M2"):
                                alarms.append(scores.get("D_ham", 0.0) > CORRECT_THRESHOLDS["D_ham_max"])
                            if config.get("M3"):
                                alarms.append(scores.get("D_ood", 0.0) > CORRECT_THRESHOLDS["D_ood_max"])
                ...
                if ep_steps >= 400:
                    print(f"  [{ep_count}] {scene}/{fault}/{ep_file}: {ep_steps} steps")
                    sys.stdout.flush()
                
                # 每 10 个 episode 保存一次中间结果
                if ep_count % 10 == 0:
                    print(f"  Saving intermediate results... ({ep_count} episodes)")
                    sys.stdout.flush()
                    save_results()
...
print(f"\nProcessed {ep_count} episodes, {total_steps} steps")
