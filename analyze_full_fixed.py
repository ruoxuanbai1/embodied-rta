...
修复内容:
1. GT: 使用数据驱动的安全范围 (从数据中学习 p1/p99)
2. R1: 与 GT 错开 (使用 p5/p95 作为预警范围)
3. R2: exceed 判断逻辑正确 (alarm = np.any(exceed < 0))
4. OOD: 使用 28 维状态 (qpos + qvel)
5. 保存频率：每 5 集
"""
...
JOINT_SAFE_RANGE = [
    ...
    (-0.30, 3.51),    # joint 4
    (-2.43, 0.00),    # joint 5
    (0.08, 2.73),     # gripper left
    (-0.16, 1.26),    # joint 0 (right)
    (-2.26, -0.96),   # joint 1 (right)
    (0.11, 1.57),     # joint 2 (right)
    (-0.16, 1.25),    # joint 3 (right)
    (-1.85, 0.00),    # joint 4 (right)
    (0.00, 0.17),     # joint 5 (right)
    (0.08, 0.18),     # gripper right
]
...
def save_results(layer_tpfp, r3_tpfp, ep_count, total_steps, r2_debug, ood_debug, gt_debug, final=False):
    ...
    summary = {
        ...
        "debug": {
            ...
            "ood": {
                ...
                "note": "OOD 分数=马氏距离 (28 维状态：qpos+qvel)"
            }
        }
    }
    ...
    if final:
        ...
        if gt_margin_summary:
            ...
            print(f"  max:  {gt_margin_summary['max']:.4f}")
            ...
            try:
                ...
                with open(filepath) as f:
                    for line in f:
                        ...
                        ood_score = compute_ood_score(qpos, qvel)  # 修正：传入 qpos+qvel
